...
public class StudentJdbcDao implements StudentDao<Integer, Student> {
    ...
    private static final String UPDATE_STUDENT = "UPDATE students SET group_id = ?, firstname = ?, lastname = ? "
	    + "WHERE student_id = ?";
    private static final String ADD_STUDENT_TO_COURSE = "INSERT INTO students_courses VALUES(?, ?)";
	...
	try (var con = connectionManager.getConnection();
	    ...
	    throw new DAOException("Failed get student by ID", e);
	}
	...
	try (var con = connectionManager.getConnection();
	    ...
	    throw new DAOException("Failed delete student", e);
	}
    }

    @Override
    public List<Student> getAllData() throws DAOException {
	...
	try (var con = connectionManager.getConnection();
	    ...
	    throw new DAOException("Failed get all students", e);
	}
    ...
    @Override
    public void save(Student studant) throws DAOException {
	try (var con = connectionManager.getConnection();
	    ...
	    throw new DAOException("Failed enroll student", e);
	}
    }
	...
	try (var con = connectionManager.getConnection();
	    ...
	    throw new DAOException("Failed update student", e);
	}
    }

    @Override
    public void addStudentToCourseById(Integer idStudent, Integer idCourse) throws DAOException {
	try (var connection = connectionManager.getConnection();
		var statement = connection.prepareStatement(ADD_STUDENT_TO_COURSE);) {
	    statement.setInt(1, (int) idStudent);
	    statement.setInt(2, (int) idCourse);
	    statement.executeUpdate();
	} catch (SQLException e) {
	    throw new DAOException("Failed added student to course", e);
	}
    }
}
